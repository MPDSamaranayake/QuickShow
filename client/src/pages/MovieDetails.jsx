...
const MovieDetails = () => {
  ...
  const getShow = async () => {
    try {
      ...
      setShow(data);
      setVoteAverage(data?.movie?.vote_average || 0);
      setRecommended(moviesList.filter(m => m._id !== id).slice(0, 4));

      // Fetch this user's rating for the movie (non-blocking)
      try {
        const ratingData = await request(`/api/movies/${id}/my-rating`);
        setUserRating(ratingData?.userRating ?? null);
      } catch (_) {
        // Rating fetch failure is non-critical
      }
    } catch (error) {
  ...
  return show ? (
    <div className="px-6 md:px-16 lg:px-40 pt-30 md:pt-50">
      <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
        ...
        <div className="relative flex flex-col gap-3">
          ...
          <StarRating
            ...
            onRated={(newAvg, newRating) => {
              setVoteAverage(newAvg);
      ...
      <div className="flex flex-wrap max-sm:justify-center gap-8 mb-20">
