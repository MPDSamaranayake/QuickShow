...
const seedDB = async () => {
    try {
        ...
        const theater = await Theater.create({
            ...
            screens: [
                { name: 'Screen 1', seatCount: 90 },
                { name: 'Screen 2', seatCount: 90 }
            ]
        });
        console.log('Inserted default theater.');

        // Insert Movies
        const movies = await Movie.insertMany(dummyShowsData);
        console.log(`Inserted ${movies.length} movies.`);

        // Insert Shows for the next 7 days for each movie
        const showPriceList = [49, 59, 69, 79, 89];
        ...
        for (let i = 0; i < 7; i++) {
            ...
            for (const movie of movies) {
                // Skip upcoming movies for immediate shows occasionally or schedule fewer
                if (movie.status === 'upcoming' && i > 3) {
                    continue; 
                }

                // Create 3 shows for each movie on each day
                const selectedTimes = showTimes.slice(0, 3);
                for (const time of selectedTimes) {
                    const [hour, minute] = time.split(':').map(Number);
                    const showDateTime = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate(), hour, minute, 0);
