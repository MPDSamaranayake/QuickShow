...
const MovieCard = ({movie}) => {
  ...
  return (
    ...
    rounded-2xl hover:-translate-y-1 transition duration-300 w-66 relative'>

                <img onClick={()=> {navigate(`/movies/${movieId}`); scrollTo(0,0)}}
        src={movie.showArtwork || movie.backdrop_path} alt="" className='rounded-lg h-52 w-full
        object-cover object-right-bottom cursor-pointer'/>

        <p className='font-semibold mt-2 truncate'>{movie.showName || movie.title}</p>

        {movie.showName && movie.showName !== movie.title && (
        ...
        <div className='flex items-center justify-between mt-4 pb-3'>
            <button onClick={()=> {navigate(`/movies/${movieId}`); scrollTo(0,0)}} className='px-4 py-2 text-xs bg-primary hover:bg-primary-dull
